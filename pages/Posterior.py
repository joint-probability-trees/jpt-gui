    ...
    [
        ...
        dbc.Row(
            [
                ...
                dbc.Col(id="q_variable_pos",
                        children=[
                            dcc.Dropdown(id="text_var_pos", options=sorted(model.varnames), value=sorted(model.varnames),
                ...
                dbc.Col(id="e_variable_pos",
                        children=[dcc.Dropdown(id={'type': 'dd_e_pos', 'index': 0}, options=sorted(model.varnames))],
                ...
                dbc.Col(children=[html.Div(id="e_option_pos", children=[
                    dbc.Button("👁️", id=dict(type='b_e_pos', index=0), disabled=True, n_clicks=0, className="me-2 mb-3",
        ...
        dbc.Row(
            [
                ...
                dbc.Col(children=[], id="pos_erg_pos", className=""),
...
@callback(
    ...
    Input("upload_tree_pos", 'contents'),
...
def post_router(upload, dd_vals, b_e, op_s, e_var, e_in, q_var, e_op, op_i):
    ...
    if cb == "upload_tree_pos" and upload is not None:
        global model
        global priors
        try:
        ...
        model = io_model
        priors = model.independent_marginals()
        q_var_n = dcc.Dropdown(id="text_var_pos", options=sorted(model.varnames), value=sorted(model.varnames),
                               multi=True, disabled=False)
        return *c.reset_gui_button(io_model, "e"), c.create_prefix_text_query(len(e_var), len(e_var)), [
            q_var_n], modal_basic_pos, False
        ...
        if len(e_var) - 1 == cb.get("index"):
            return *c.add_selector_to_div_button(model, e_var, e_in, e_op, "e_pos", cb.get("index") + 1), \
                c.create_prefix_text_query(len(e_var), len(e_var)), q_var, modal_basic_pos, False
